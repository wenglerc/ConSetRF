...
# S0.2 = S0[[2]]
#
# U <- ConfSet(data.all[[1]], S, partitions, alpha, level, equal = T,
#              pmethod = "tGKF")
...
results.gkf <-
    foreach(data = data.all, .packages = c("dplyr", "ConfSetRF")) %dopar% {
        ...
        lapply(samplesize.list, function(N) {
            ConfSet(data[, 1:N], S, partitions, alpha, level, equal = T,
                    pmethod = "tgkf")
        })
    }
stopCluster(cluster)
